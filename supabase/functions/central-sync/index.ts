...
async function rateLimitedFetch(url: string, options: RequestInit, type: "gc" | "auvo"): Promise<Response> {
  if (type === "gc") {
    ...
    const protectedHeaders = new Headers(options.headers);
    protectedHeaders.set("usuario-id", GC_API_USER_ID);
    const protectedOptions: RequestInit = { ...options, headers: protectedHeaders };
