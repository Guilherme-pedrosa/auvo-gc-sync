...
RETURNS TABLE (auvo_task_id text, coluna text, posicao integer)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.kanban_orcamentos_cache AS cache (
      auvo_task_id, dados, coluna, posicao, atualizado_em
    ) VALUES (
      v_row.auvo_task_id, v_row.dados, v_row.auto_coluna, v_row.posicao, now()
    )
    ON CONFLICT (auvo_task_id) DO UPDATE SET
      dados = EXCLUDED.dados,
      coluna = CASE
        WHEN EXISTS (
          SELECT 1 FROM public.kanban_resolution_details d
