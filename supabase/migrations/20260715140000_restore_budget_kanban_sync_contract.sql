-- Restore the database contract required by the Budget Kanban Edge Function.
-- The original 20260714190000 migration reached GitHub but was not applied to
-- production. Keep this migration idempotent so environments that did apply it
-- can safely receive the repair as well.
...
ALTER TABLE public.kanban_sync_meta
  DROP CONSTRAINT IF EXISTS kanban_sync_meta_sync_status_check;

ALTER TABLE public.kanban_sync_meta
  ADD CONSTRAINT kanban_sync_meta_sync_status_check
  CHECK (sync_status IN ('idle', 'running', 'succeeded', 'failed'));

-- A historical detail is active only when the card is still in the manual
-- resolved column. This avoids resurrecting cards that were reopened before
-- the explicit `ativo` flag existed.
