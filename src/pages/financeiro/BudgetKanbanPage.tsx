    ...
    if (hasSavedPositions) {
      ...
      if (feitoHojeCol) {
        ...
        if (target) {
          ...
          for (const card of evicted) {
            const destId = resolveSystemColumn(card);
            let dest = cols.find((c) => c.id === destId);
            if (!dest) {
              dest = { id: destId, title: destId === "os_realizada" ? "🔧 OS Realizada" : destId.startsWith("orc_") ? `💰 ${destId.replace("orc_", "").replace(/_/g, " ")}` : destId, items: [] };
              cols.push(dest);
            }
  ...
  const handleChatSend = useCallback(async () => {
    ...
    try {
      ...
      if (error || result?.error || result?.errorCode) {
        const aiErr = await parseAiError(result, error);
        toast.warning(aiErr.message);
        setChatMessages(prev => [...prev, { role: "assistant", content: "⚠️ Chat indisponível no momento. Use o checklist operacional e tente novamente em instantes." }]);
        return;
      }
