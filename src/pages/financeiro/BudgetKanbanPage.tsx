...
import {
  ...
  FileText, Plus, GripVertical, Trash2, Edit2, Check, X
} from "lucide-react";
...
const DEFAULT_COLUMNS = [
  ...
  { id: "concluido", title: "✅ Orçamento Realizado" },
];
...
export default function BudgetKanbanPage() {
  ...
  // Reset columns when date changes
  ...
  const filteredColumns = useMemo(() => {
    return columns.map((col) => ({
      ...
      items: col.items.filter((item) => {
        ...
        if (filterCliente && !item.cliente.toLowerCase().includes(filterCliente.toLowerCase()) &&
            !item.orientacao.toLowerCase().includes(filterCliente.toLowerCase()) &&
            !(item.gc_orcamento?.gc_cliente || "").toLowerCase().includes(filterCliente.toLowerCase())) return false;
        return true;
      }),
    }));
  }, [columns, filterTecnico, filterCliente]);
  ...
  // Delete column (moves items to "a_fazer")
  const deleteColumn = useCallback((columnId: string) => {
    setColumns((prev) => {
      const col = prev.find((c) => c.id === columnId);
      if (!col) return prev;
      const aFazer = prev.find((c) => c.id === "a_fazer");
      return prev
                                ...
                                <div className="flex items-start gap-1 px-3 py-2">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-sm font-medium text-foreground mt-1 truncate">
                                      {item.gc_orcamento?.gc_cliente || item.cliente || item.orientacao.substring(0, 50)}
                                    </p>
                                    ...
                                    <div className="flex items-center gap-2 mt-2">
                                      ...
                                      <button
                                        onClick={() => setExpandedCard(
                                          expandedCard === item.auvo_task_id ? null : item.auvo_task_id
                                        )}
                                        className="ml-auto text-[10px] text-muted-foreground hover:text-foreground"
                                      >
                                        {expandedCard === item.auvo_task_id ? "Menos" : "Detalhes"}
                                      </button>
                                    </div>

                                    {/* Expanded details */}
                                    {expandedCard === item.auvo_task_id && (
                                      <div className="mt-2 pt-2 border-t space-y-1">
                                        <p className="text-xs text-muted-foreground whitespace-pre-wrap">
                                          {item.orientacao}
                                        </p>
                                        {item.questionario_respostas
                                          .filter((r) => r.reply && !r.reply.startsWith("http"))
                                          .map((r, i) => (
                                            <div key={i} className="text-xs">
                                              <span className="font-medium text-foreground">{r.question}:</span>{" "}
                                              <span className="text-muted-foreground whitespace-pre-wrap">{r.reply}</span>
                                            </div>
                                          ))}
                                      </div>
                                    )}
