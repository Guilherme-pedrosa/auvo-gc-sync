...
import { RefreshCw, Play, Eye, ChevronDown, ChevronRight, ArrowLeft } from "lucide-react";
...
const AuvoSyncPage = () => {
  ...
  const executarSync = async (dryRun = false) => {
    ...
    try {
      ...
      toast.success(
        dryRun
          ? `Dry Run concluído: ${data.osCandidatas} OS candidatas, ${data.semPendencia} prontas para atualizar`
          : `Sync concluído: ${data.atualizadas} OS atualizadas, ${data.erros} erros`
      );
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Automação de fechamento de OS baseada no status das tarefas do Auvo
          </p>
        </div>
      </div>
            ...
            {lastSync ? (
              <div className="space-y-2 text-sm">
                <p>
                  ...
                  {format(new Date(lastSync.executado_em), "dd/MM/yyyy HH:mm:ss", { locale: ptBR })}
                </p>
                <p>
                  <span className="text-muted-foreground">OS Candidatas:</span> {lastSync.os_candidatas}
                </p>
                <p>
                  <span className="text-muted-foreground">Atualizadas:</span>{" "}
                  <span className="text-green-600 font-medium">{lastSync.os_atualizadas}</span>
                </p>
                <p>
                  <span className="text-muted-foreground">Erros:</span>{" "}
                  <span className={lastSync.erros > 0 ? "text-red-600 font-medium" : ""}>{lastSync.erros}</span>
                </p>
                <p>
                  <span className="text-muted-foreground">Duração:</span> {lastSync.duracao_ms}ms
                </p>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => queryClient.invalidateQueries({ queryKey: ["auvo-sync-logs"] })}
          >
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-center">🔍 Não Encontradas</TableHead>
                      ...
                      <CollapsibleTrigger asChild>
                        ...
                        >
                          ...
                          <TableCell className="text-sm">
                            {format(new Date(log.executado_em), "dd/MM HH:mm", { locale: ptBR })}
                          </TableCell>
                          ...
                          <TableCell className="text-center">
                            <span className={log.erros > 0 ? "text-red-600 font-medium" : ""}>
                              {log.erros}
                            </span>
                          </TableCell>
                          <TableCell className="text-center">
                            {log.dry_run && <Badge variant="secondary">Sim</Badge>}
                          </TableCell>
                                ...
                                <TableBody>
                                  {(log.detalhes as LogDetail[]).map((d, i) => (
                                    <TableRow key={i}>
                                      ...
                                      <TableCell className="text-xs max-w-xs truncate" title={d.detalhe}>
                                        {d.detalhe}
                                      </TableCell>
                                    </TableRow>
                                  ))}
