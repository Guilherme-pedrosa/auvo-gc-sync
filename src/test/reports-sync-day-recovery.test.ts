...
describe("continuidade por dia do Controle OS", () => {
  it("segue após o 404 do quarto dia, mantém pendência e soma somente seis dias confirmados", async () => {
    ...
    const onSaved = vi.fn();
    const result = await syncReportsInSteps(invoke, { days, situationIds: ["7063705"], onProgress, onWarnings, onSaved });
    expect(invoke.mock.calls.filter(([, { body }]) => body.reports_only).map(([, { body }]) => body.start_date))
      .toEqual(days.map(day => day.start));
    expect(maximumConcurrent).toBe(1);
    expect(result).toMatchObject({ tasks: 12, saved: 12, orders: 1, incomplete: true });
    ...
    expect(onSaved).toHaveBeenCalledTimes(8); // Two GC batches plus six confirmed Auvo dates.
    expect(onProgress.mock.lastCall).toEqual([expect.stringMatching(/Sincronização parcial: 1 dia Auvo não confirmado/), 8]);
    expect(onProgress).not.toHaveBeenCalledWith("Todos os lotes foram concluídos e gravados.", expect.anything());
  });

  it("mantém OS sem permissão e dias Auvo não confirmados na mesma lista tipada", async () => {
    ...
    const onWarnings = vi.fn();
    const result = await syncReportsInSteps(invoke, { days, situationIds: ["7063705"], onProgress: vi.fn(), onWarnings });
    expect(result.warnings[0]).toEqual(osWarning);
    expect(result.warnings[1]).toMatchObject({ kind: "auvo_day", start_date: "2026-07-04" });
    expect(onWarnings.mock.calls.map(([warnings]) => warnings.length)).toEqual([1, 2]);
    expect(reportsSyncPendingSummary(result.warnings)).toBe("1 OS pendentes de conferência; 1 dia Auvo não confirmado");
  });
  ...
  it("mantém a causa JSON do timeout do dia sem encerrar os próximos dias", async () => {
    const invoke = vi.fn(async (_name, { body }) => body.start_date === "2026-07-04"
      ...
      : success(body));
    const result = await syncReportsInSteps(invoke, { days, situationIds: ["7063705"], onProgress: vi.fn() });
  ...
  it("todos os dias incompletos terminam como resultado parcial e zero tarefas confirmadas", async () => {
    const invoke = vi.fn(async (_name, { body }) => body.reports_only
      ...
      : success(body));
    const onProgress = vi.fn();
    const result = await syncReportsInSteps(invoke, { days, situationIds: ["7063705"], onProgress });
    expect(result).toMatchObject({ tasks: 0, saved: 0, incomplete: true });
    expect(result.warnings).toHaveLength(7);
    expect(onProgress.mock.lastCall).toEqual([expect.stringContaining("7 dias Auvo não confirmados"), 2]);
  });
  ...
  it("cancelamento durante um dia com falha interrompe a fila em vez de virar pendência", async () => {
    ...
    const onWarnings = vi.fn();
    await expect(syncReportsInSteps(invoke, { days, situationIds: ["7063705"], onProgress: vi.fn(), onWarnings,
  ...
  it("AbortError explícito do provedor também encerra os próximos dias", async () => {
    ...
    await expect(syncReportsInSteps(invoke, { days, situationIds: ["7063705"], onProgress: vi.fn() })).rejects.toThrow("Cancelado");
    expect(invoke.mock.lastCall?.[1].body.start_date).toBe("2026-07-04");
  });
  ...
  it.each([
    ...
    const onWarnings = vi.fn();
    await expect(syncReportsInSteps(invoke, { days, situationIds: ["7063705"], onProgress: vi.fn(), onWarnings })).rejects.toThrow();
    expect(invoke.mock.lastCall?.[1].body.start_date).toBe("2026-07-04");
    expect(onWarnings).not.toHaveBeenCalled();
  });
});
