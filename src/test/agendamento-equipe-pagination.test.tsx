...
describe("carregamento completo da grade Agendamento Equipe", () => {
  it("preserva as tarefas de terça e as previsões de quarta de Luiz e Igor após recarregar uma escala com mais de mil registros", async () => {
    // Incidente de 09/09/2026: os quatro registros existiam no banco, mas
    // desapareciam na leitura sem paginação. IDs e datas conferidos no caso real.
    const cases = [
      { id: "f0a386b9-5b83-4c4d-a8c3-ee3cdede7ca2", data: "2026-09-08", colaborador_nome: "LUIZ VANDE GONÇALVES", auvo_task_id: "79424514", origem: "AUVO", previsao_continuidade: false },
      { id: "0d200775-f567-40c1-a710-36019da6132e", data: "2026-09-08", colaborador_nome: "IGOR DE ABREU COSTA", auvo_task_id: "79424423", origem: "AUVO", previsao_continuidade: false },
      { id: "0604a21b-64a9-4804-974e-9638db8d01ba", data: "2026-09-09", colaborador_nome: "LUIZ VANDE GONÇALVES", auvo_task_id: null, origem: "MANUAL", previsao_continuidade: true, previsao_tipo: "CONTINUACAO" },
      { id: "4a3d95c8-d5ae-4ee7-b73b-efcac031e816", data: "2026-09-09", colaborador_nome: "IGOR DE ABREU COSTA", auvo_task_id: null, origem: "MANUAL", previsao_continuidade: true, previsao_tipo: "CONTINUACAO" },
    ];
    db.tables.agenda_agendamentos = [
      ...Array.from({ length: 1100 }, (_, i) => ({ ...schedule(i), data: "2026-08-15" })),
      ...cases.map(row => ({ ...schedule(2000), cliente: "SODEXO DO BRASIL COMERCIAL S.A. HYPER MARCAS", ...row })),
    ];
    db.tables.tarefas_central = [
      { mirror_key: "79424514::os:::orc:", auvo_task_id: "79424514", status_auvo: "Pausada", atualizado_em: "2026-09-09T20:00:00Z" },
      { mirror_key: "79424423::os:::orc:", auvo_task_id: "79424423", status_auvo: "Pausada", atualizado_em: "2026-09-09T20:00:00Z" },
    ];
    const { wrapper } = setup();
    const { result } = renderHook(() => useAgendaSemana(days), { wrapper });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    for (const row of cases) expect(result.current.data?.agendamentos.find(item => item.id === row.id)).toMatchObject(row);
    expect(result.current.data?.agendamentos.find(item => item.auvo_task_id === "79424514")?.status_auvo).toBe("Pausada");
